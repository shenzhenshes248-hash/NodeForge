...
本地测试使用模拟 GitHub 下载及临时 Ed25519 密钥验证发现/验签；Shell fixtures 验证 no-op、复制/发布/入口切换/前后健康检查失败回退、成功后版本和 Xray 文件摘要不变。真实 GitHub/VPS 更新尚未验收。

已发布的 Phase 4 `v0.2.0-dev` 不含 update 命令或 verifier runtime 资源，不能追溯获得此功能。本阶段需要随后发布含本实现的版本再部署验收；本轮不发布、不操作 VPS，VERSION 不变。bootstrap 文件仅因共享 verifier/清单变化重新生成，流程未改造。
