...
仅实现 Debian 12 amd64 的 bootstrap，不新增安装器、更新或版本切换。正式入口（本轮尚未 push，因此此命令尚未上线）：
...
本地测试用模拟下载、临时测试公钥和仅写标记的 fixture installer，验证成功、安装器失败、三个下载失败、三类篡改、已签名的越界/链接/缺项 bundle 拒绝及临时目录清理，不连接 GitHub/VPS。发布上线仍需后续授权：推送 bootstrap、创建对应 tag/Release、上传正式私钥签名的三个资产；本轮不执行这些操作。
